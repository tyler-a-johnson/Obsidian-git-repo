...
$a_{i}$ (sometimes L) = the distance from $\bar{Z}_{i}$ to $\hat{Z}_{i+1}$ measured along $\hat{X}_{i}$

$\alpha_{i}$ = the angle from $\hat{Z}_{i}$ to $\hat{Z}_{i+1}$ measured about $\hat{X}_{i}$
